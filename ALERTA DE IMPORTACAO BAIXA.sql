...
DROP TABLE IF EXISTS #IMP19BAIXA
...
WHERE I.ARQ_IMP LIKE '%5401%' AND I.DATA_IMP BETWEEN CAST(DATEADD(day,0, GETDATE()) AS DATE) AND CAST(GETDATE()+1 AS DATE) AND I.ARQ_IMP LIKE '%EXC_BAI%'
...
DECLARE
	...
	SET @ARQMOV = (SELECT TOP 1 ARQ_IMP FROM #IMP19BAIXA)
	SET @iNICIO = (SELECT TOP 1 INICIO_IMPORTACAO FROM #IMP19BAIXA)
	SET @FIM = (SELECT TOP 1 FIM_IMPORTACAO FROM #IMP19BAIXA)
	SET @DEVOLUCAOCONTRATO = (SELECT TOP 1 CONTRATOS FROM #IMP19BAIXA WHERE DESCRICAO LIKE '%Devolução%')
	SET @DEVOLUCAOPARCELAS = (SELECT TOP 1 PARCELAS FROM #IMP19BAIXA WHERE DESCRICAO LIKE '%Devolução%')
