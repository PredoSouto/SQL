...
DROP TABLE IF EXISTS #IMP019MOV
...
WHERE I.ARQ_IMP LIKE '%SE01%' AND I.DATA_IMP BETWEEN CAST(DATEADD(day,0, GETDATE()) AS DATE) AND CAST(GETDATE()+1 AS DATE) AND I.ARQ_IMP LIKE '%MOV_FIN%'
...
DECLARE
	...
	@DEVOLUCAOCONTRATO AS BIGINT,--QNT DE DEVOLUCAO POR CONTRATOS
	...
	SET @ARQMOV = (SELECT TOP 1 ARQ_IMP FROM #IMP019MOV)
	SET @iNICIO = (SELECT TOP 1 INICIO_IMPORTACAO FROM #IMP019MOV)
	SET @FIM = (SELECT TOP 1 FIM_IMPORTACAO FROM #IMP019MOV WHERE ARQ_IMP LIKE '%MOV_FIN%')
	SET @DEVOLUCAOCONTRATO = (SELECT TOP 1 CONTRATOS FROM #IMP019MOV WHERE DESCRICAO LIKE '%Devolução%')
	SET @DEVOLUCAOPARCELAS = (SELECT TOP 1 PARCELAS FROM #IMP019MOV WHERE DESCRICAO LIKE '%Devolução%')
	SET @ATUALIZACAOCONTRATOS = (SELECT TOP 1 CONTRATOS FROM #IMP019MOV WHERE DESCRICAO LIKE '%Atualização%')
	SET @ATUALIZACAOPARCELAS = (SELECT TOP 1 PARCELAS FROM #IMP019MOV WHERE DESCRICAO LIKE '%Atualização%')
